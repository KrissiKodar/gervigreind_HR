...
class backtracking_with_prechecking(nonogram_solver):
	...
	def start_search(self, puzzle):
		...
		for key, value in self.assignment.items():
			if value != None:
				puzzle[key] = value
				continue
			non_determined_cells += 1
		print("before starting search:")
		self.print_assignment(self.assignment)
		print("size of old state space = ", "{:.2e}".format(2**(self.n_cells)))
		print("non_determined_cells", non_determined_cells)
		print("size of new state space = ", "{:.2e}".format(2**(non_determined_cells)))
		print("------------------------")
		return self.backtracking_search(puzzle)
	...
	def backtracking_search(self, puzzle):
		...
		values_list = [1, 0]
		#random.shuffle(values_list)
		#values_list = random.choices([[1, 0], [0, 1]], weights=[0.50, 0.50])[0]
		for i in values_list:
			temp_puzzle = puzzle.copy()
			temp_puzzle[current_cell] = i
			assignment_temp[current_cell] = i
...
class forward_checking_attempt_2(nonogram_solver):
	def __init__(self, puzzle, row_constraints, col_constraints):
		...
		self.assignments_before_search = {}
		self.states_visited = 0

		# pre-constrain check
		for i in range(self.n_rows):
			for j in range(self.n_cols):
				self.assignments_before_search[(i, j)] = None
		...
		for i, row in enumerate(puzzle):
			...
			row_shifted = row_copy[-shift_by:] + row_copy[:-shift_by]
			result = [1 if ((row_copy[i] != 0 and row_shifted[i] != 0) and (row_copy[i] == row_shifted[i])) else 0 for i in range(min(len(row_copy), len(row_shifted)))]
			# where result is 1 make self.assignments_before_search[(i, m)] = 1
			for m, value in enumerate(result):
				if value == 1:
					self.assignments_before_search[(i, m)] = 1
				...
				for k in range(len(row)):
					...
					if result[k] == 1:	
						...
						if k+1 < len(result) and result[k+1] == 0:
							group_fill += 1	
				padded_cells = self.pad_list(test, padding_list)
				# where padded cells are 0 make self.assignments_before_search[(i, m)] = 0
				for m, value in enumerate(padded_cells):
					if value == 0:
						self.assignments_before_search[(i, m)] = 0
		...
		for i, col in enumerate(puzzle.T):
			...
			result = [1 if ((col_copy[i] != 0 and col_shifted[i] != 0) and (col_copy[i] == col_shifted[i])) else 0 for i in range(min(len(col_copy), len(col_shifted)))]
			# where result is 1 make self.assignments_before_search[(m, i)] = 1
			for m, value in enumerate(result):
				if value == 1:
					self.assignments_before_search[(m, i)] = 1
			...
			if len(col_groups) == len(self.col_constraints[i]):
				...
				#print("col", i)
				#print("padded_cells", padded_cells)
				# where padded cells are 0 make self.assignments_before_search[(m, i)] = 0
				for m, value in enumerate(padded_cells):
					if value == 0:
						self.assignments_before_search[(m, i)] = 0


		temp_puzzle = np.zeros((self.n_rows, self.n_cols))
		for key, value in self.assignments_before_search.items():
			if value != None:
				temp_puzzle[key] = value
		...
		for i, row in enumerate(temp_puzzle):
			if np.sum(row) == self.row_sums[i] and super().count_groups(row) == self.row_groups[i]:
				# assign values from temp_puzzle row to self.assignments_before_search
				for j, value in enumerate(row):
					self.assignments_before_search[(i, j)] = value
		for i, col in enumerate(temp_puzzle.T):
			if np.sum(col) == self.col_sums[i] and super().count_groups(col) == self.col_groups[i]:
				# assign values from temp_puzzle col to self.assignments_before_search
				for j, value in enumerate(col):
					self.assignments_before_search[(j, i)] = value
	...
	def is_consistent(self, puzzle, assignment):
		#row sums and groups
		for i, row in enumerate(puzzle):
			sum_row = np.sum(row)
			row_groups = super().count_groups(row)
			...
			if sum_row == self.row_sums[i] and row_groups != self.row_groups[i]:
				return False
			# count number of None in assignment for this row
			none_count = 0
			for j in range(self.n_cols):
	...
	def start_search(self, puzzle):
		...
		for key, value in self.assignments_before_search.items():
			...
			non_determined_cells += 1
		print("before starting search:")
		self.print_assignment(self.assignments_before_search)
		print("size of old state space = ", "{:.2e}".format(2**(self.n_cells)))
		print("non_determined_cells", non_determined_cells)
		print("size of new state space = ", "{:.2e}".format(2**(non_determined_cells)))
		print("------------------------")
		return self.backtracking_search(puzzle, self.assignments_before_search)

	def forward_checking(self, puzzle, assignment):
		for i, row in enumerate(puzzle):
			if np.sum(row) == self.row_sums[i] and super().count_groups(row) == self.row_groups[i]:
				# assign values from temp_puzzle row to self.assignment
				for j, value in enumerate(row):
	...
	def backtracking_search(self, puzzle, assignment):
		...
		for key, value in assignment.items():
			if value == None:
				...
				#print("current cell = ", current_cell)
				break
		# pick a random cell in assignment which has a value of None
		#current_cell = random.choice([key for key, value in assignment.items() if value == None])
		...
		for i in values_list:
			temp_puzzle = puzzle.copy()
			temp_puzzle[current_cell] = i
			assignment[current_cell] = i
			if self.is_consistent(temp_puzzle, assignment):
				assignment[current_cell] = i
				result = self.backtracking_search(temp_puzzle, assignment)
				if result != False:
					return result
				assignment[current_cell] = None
		#print("backtracking...")
		return False
